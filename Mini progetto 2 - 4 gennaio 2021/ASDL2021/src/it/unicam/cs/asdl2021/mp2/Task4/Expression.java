...
public class Expression {
     ...
     * @throws IllegalArgumentException if the format is not satisfied in the given text
     */
    public Expression(String text) {
        if (text == null) throw new NullPointerException("Expression con testo null");
        // Eliminate all the white spaces from the text
        this.text = text.replaceAll("\\s+", "");
        if (this.text.length() == 0) throw new IllegalArgumentException("Expression vuota");
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            try {
                ...
                if (!c.equals("*") && !c.equals("+"))
                    throw new IllegalArgumentException("Formato della espressione non valido");
        ...
        if (this.items[0].getType() != ItemType.DIGIT)
            throw new IllegalArgumentException("L'espressione non inizia con una cifra");
        ...
        for (int i = 0; i < this.items.length - 1; i++) {
            if (this.items[i].getType() == ItemType.DIGIT && this.items[i + 1].getType() != ItemType.OPERATOR)
                throw new IllegalArgumentException("Cifra non seguita da un operatore");
            if (this.items[i].getType() == ItemType.OPERATOR && this.items[i + 1].getType() != ItemType.DIGIT)
                throw new IllegalArgumentException("Operatore non seguito da una cifra");
        }
        // Check that the formula ends with a digit
        if (this.items[this.items.length - 1].getType() != ItemType.DIGIT)
            throw new IllegalArgumentException("L'espressione non termina con una cifra");
    }
