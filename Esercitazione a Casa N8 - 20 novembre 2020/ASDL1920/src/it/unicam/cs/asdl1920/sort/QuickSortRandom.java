...
public class QuickSortRandom<E extends Comparable<E>>
    ...
    private PartitionResult partition(List<E> a, int p, int r) {
        // Inizializziamo il pivot con uno degli elementi a caso tra le
        // posizioni p ed r
        // Scelgo una posizione a caso tra p ed r (inclusi)
        int shift = randomGenerator.nextInt(r - p + 1);
        // sposto l'elemento in posizione p+shift con l'elemento in posizione r,
        // in modo che diventi lui il pivot; a meno che p+shift non sia già r
        if (p + shift != r) {
            // scambio
            E appoggio = a.get(p + shift);
            a.set(p + shift, a.get(r));
            a.set(r, appoggio);
        }
        // Inizializziamo x con il pivot, in posizione r
        E x = a.get(r);
